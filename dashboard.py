...
# --- 2. CORPORATE THEME CSS (WHITE MODE) ---
st.markdown("""
    <style>
        .stApp {
        ...
        /* FIX: Only hide the 3-dots menu and footer, NOT the header */
        ...
        .ticker-wrap { position: fixed; bottom: 0; left: 0; width: 100%; background: #0F2A4A; height: 35px; line-height: 35px; z-index: 100; }
...
# --- 3. ASSETS & API ---
LOGO_URL = "my_logo.png"  # Replace with your actual logo URL or path
...
# --- 5. STATIC VALUES INITIALIZATION ---
if 'static_co2' not in st.session_state:
    st.session_state['static_co2'] = "*"
if 'static_trees' not in st.session_state:
    st.session_state['static_trees'] = "*"
if 'client_name' not in st.session_state:
    st.session_state['client_name'] = "ACME Industries Ltd."
if 'client_loc' not in st.session_state:
    st.session_state['client_loc'] = "New York, HQ"
if 'history_data' not in st.session_state:
    st.session_state['history_data'] = pd.DataFrame(columns=['created_at', 'co2'])

# --- 6. PAGE: ADMIN SETTINGS ---
if page == "⚙️ Admin Settings":
    st.title("⚙️ System Configuration")
    st.markdown("---")
    st.info("Update dashboard metadata below.")
    
    with st.form("admin_form"):
        st.subheader("Client Details")
        c1, c2 = st.columns(2)
        with c1:
            new_client = st.text_input("Client Name", value=st.session_state['client_name'])
        with c2:
            new_loc = st.text_input("Location", value=st.session_state['client_loc'])
            
        st.subheader("Impact Data")
        c3, c4 = st.columns(2)
        with c3:
            new_co2 = st.text_input("Cumulative CO2 Absorbed (kg)", value=st.session_state['static_co2'])
        with c4:
            new_trees = st.text_input("Tree Equivalence", value=st.session_state['static_trees'])
...
# --- 7. PAGE: DASHBOARD ---
    ...
    if not new_df.empty:
        ...
        spacer = "&nbsp;" * 20
        t_text = f"SYSTEM STATUS: ONLINE {spacer} DEVICE: 100...7C62 {spacer} AQI: {curr.get('AQI',0)} {spacer} CO2: {curr.get('co2',0)} PPM {spacer} UPDATED: {datetime.now().strftime('%H:%M:%S')}" * 5
        st.markdown(f'<div class="ticker-wrap"><div class="ticker-move"><span class="ticker-item">{t_text}</span></div></div>', unsafe_allow_html=True)

    # HEADER SECTION
    st.markdown("<br>", unsafe_allow_html=True)
    
    # 1. ADJUSTED COLUMNS: Logo gets balanced space
    c_logo, c_title, c_info = st.columns([2, 3, 3])
    
    with c_logo:
        # 2. ADJUSTED LOGO WIDTH (Reduced to 220px)
        ...
        <div class="info-box">
            <div style="display: flex; justify-content: space-between;">
                <div>
                    ...
                    <div class="info-value">{st.session_state['client_name']}</div>
                </div>
                <div>
                    <div class="info-label">Location</div>
                    <div class="info-value">{st.session_state['client_loc']}</div>
                </div>
                <div>
                    <div class="info-label">Device ID</div>
                    <div class="info-value" style="font-family: monospace;">100...7C62</div>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    if not new_df.empty:
        ...
        with c1: st.metric("AQI Index", f"{curr.get('AQI',0)}")
        ...
        st.warning("Establishing Secure Connection...")
    ...
    with i1:
        st.markdown(f"""
        <div class="css-1r6slb0" style="border-left: 5px solid #28a745;">
            <div style="font-size: 12px; color: #888; font-weight: bold; text-transform: uppercase;">Cumulative CO2 Absorbed</div>
    ...
    with i2:
        st.markdown(f"""
        <div class="css-1r6slb0" style="border-left: 5px solid #28a745;">
            <div style="font-size: 12px; color: #888; font-weight: bold; text-transform: uppercase;">Tree Equivalence</div>
    ...
    with col_chart:
        ...
        if not plot_df.empty:
            ...
            fig.add_trace(go.Scatter(
                x=plot_df['created_at'], y=plot_df['co2'], 
                mode='lines', name='CO2',
                line=dict(width=3, color='#0F2A4A'),
                fill='tozeroy', fillcolor='rgba(15, 42, 74, 0.1)'
            ))
            fig.update_layout(
        ...
        if not new_df.empty:
            ...
            <div style="background: white; border: 1px solid #ddd; padding: 15px; border-radius: 8px;">
                <table class="packet-table">
                    ...
                    <tr><td class="packet-key">Signal</td><td class="packet-val" style="color:green;">Excellent</td></tr>
                </table>
            </div>
            """, unsafe_allow_html=True)
